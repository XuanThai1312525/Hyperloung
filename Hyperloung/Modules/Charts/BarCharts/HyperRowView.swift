...
class HyperRowView: UIView {
    struct Appearance {
        ...
        var descriptionWidth: CGFloat = 70
    ...
    func drawChart() {
        let availableWidth = frame.width - appearance.descriptionWidth - appearance.leadingSpace - 10
        let backgroundFrame = CGRect(x: appearance.leadingSpace + 10, y: frame.maxY / 2 - appearance.lineHeight / 2, width: availableWidth, height: appearance.lineHeight)
        ...
        let highlightFrame = CGRect(x: appearance.leadingSpace + 10, y: frame.maxY / 2 - appearance.lineHeight / 2, width: highlightWidth, height: appearance.lineHeight)
        ...
        if let highlightDescription = appearance.highlightDescription {
            let attributeString = NSAttributedString(string: highlightDescription, attributes: [.foregroundColor: UIColor.white, .font: FontFamily.customFont.displayFontWithSize(12, attributeType: .bold)])
            let textSize = attributeString.size()
            highlightLabel.frame = CGRect(x: appearance.leadingSpace + 10 + 6 , y: frame.height / 2 - textSize.height / 2, width: textSize.width, height: textSize.height)
        ...
        if let leadingDescription = appearance.leadingDescription {
            let attributeString = NSAttributedString(string: leadingDescription, attributes: [.foregroundColor: appearance.leadingTextColor, .font: FontFamily.customFont.displayFontWithSize(12, attributeType: .bold)])
            let textSize = attributeString.size()
